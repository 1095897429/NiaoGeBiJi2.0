...
public class BitmapUtils {
    ...
    private static Bitmap compressImage(Bitmap image) {
        ...
        int options = 100;
        KLog.d("tag","开始 baos.toByteArray().length / 1024 " + baos.toByteArray().length / 1024);
        while (baos.toByteArray().length / 1024 > 15  * 1024) {
            baos.reset();
            image.compress(Bitmap.CompressFormat.JPEG, options, baos);
            options -= 10;
        }
        KLog.d("tag"," 结束 baos.toByteArray().length / 1024 " + baos.toByteArray().length / 1024);
