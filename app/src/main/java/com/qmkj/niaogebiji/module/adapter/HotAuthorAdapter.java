...
//        //影响数 x>=100000，展示10w+
        if(!TextUtils.isEmpty(item.getHit_count_weekly())){
            long count = Long.parseLong(item.getHit_count_weekly());
            if(count < 100000 ){
                helper.setText(R.id.effect_num,"影响力 " + item.getHit_count_weekly());
            }else{
                double temp = count  ;
                //1.将数字转换成以万为单位的数字
