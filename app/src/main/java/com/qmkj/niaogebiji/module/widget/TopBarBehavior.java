 ...
 * 描述:不加构造函数会出现问题
 */
public class TopBarBehavior extends AppBarLayout.ScrollingViewBehavior {
    ...
    public void onStopNestedScroll(@NonNull CoordinatorLayout coordinatorLayout, @NonNull View child, @NonNull View target, int type) {
        ...
        LinearLayout part =  coordinatorLayout.findViewById(R.id.rl_top_part);
        Rect localRect = new Rect();
        boolean isVisible = part.getLocalVisibleRect(localRect);
        KLog.d("tag","头部信息的状态 " + isVisible );
        //在看不见的情况下，去显示搜索图片
        if(!isVisible){
            EventBus.getDefault().post(new ShowTopTitleEvent("1"));
        }else{
            EventBus.getDefault().post(new ShowTopTitleEvent("0"));
        }
    }
}
